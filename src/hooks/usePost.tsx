// Do not edit this file directly - Generated on 6/12/2023. 

import { HeatmapProps, PostProps } from "../types"

const directory: PostProps[] = [
    {
        filename: "attention",
        title: "Attention Seeking Behavior",
        description: "Constantly surrounded by people that do actions merely looking to get a response they must be treated like little siblings A stream of frustration and annoyance those with an Attention Seeking Behavior rarely provide any level of i",
        image: "/cdn/place.png",
        content: "Constantly surrounded by people that do actions merely looking to get a response, they must be treated like little siblings. A stream of frustration and annoyance, those with an [[Attention Seeking Behavior]] rarely provide any level of impact or perspective.\n\n```javascript\n/// pack the target address and calldata together and sign them.\nlet rawData = web3.eth.abi.encodeParameters(\n  [\"address\", \"bytes\"],\n  [organization.address, data]\n);\n\n// hash the data.\nlet hash = web3.utils.soliditySha3(rawData);\n\n// sign the hash.\nlet signature = web3.eth.sign(hash, signer);\n```\n\nInstead, it <author author=\"nftchance\" created=\"03/02/2023\" note=\"Revolves around a system where they are the sun of their universe rather than ever having realized how to interact with the galaxies beyond them.\" side=\"left\">revolves around a system where they are the sun of their universe rather than ever having realized how to interact with the galaxies beyond them.</author>\n\nLeft to `wonder why` they are not like the rest, they resort to [Attention Seeking Behavior](/post/appetite/) because it's the only outlet that continually gets them a response. Yet, the feat they only receive the response only continues to perpetuate the outcome.\n\n---\n\n| Tables        |      Are      |  Cool |\n| ------------- | :-----------: | ----: |\n| col 3 is      | right-aligned | $1600 |\n| col 2 is      |   centered    |   $12 |\n| zebra stripes |   are neat    |    $1 |\n\nAlthough annoying, these people are also a sign that you are reaching the point of escape velocity. For if these people are not around you, you are still miles away from having an audience that is willing to become evangelists.\n\n![Step into the vast world of a decentered identity. Step into the vast world of a decentered identity. Step into the vast world of a decentered identity.](/cdn/decentered.png)\n\n**How can you easily determine if you are this phase?**\n\n- Are you getting a lot of bug reports that include zero information and just bring a bug like a child?\n- Are you experiencing situations where the user is so incredibly inept that they don't even have the ability to verbalize or communicate an issue?\n- Are you getting people saying things that make absolutely zero sense yet they say it confidently?\n- Are you experiencing large amounts of misperceptions from the average user?\n\nThese are the things that you will not see come in on GitHub. This phase is the one that you will feel deep in your gut that bring the first murmurings of abandoning yet another idea. The type of response and conversations that frustrate you to a feeling that is disgusting and ridiculous.\n\n> \"But, these displays are precisely why these individuals are participants of your network rather than your network.\"\n>\n> - **_Vasa_**\n\nInterestingly, frustration at these people shows you still have more growing to do. Vasa of [Gem.xyz](https://gem.xyz) has shown through years of effort and dedication that when you take care of these people, they will take care of you. Important to understand though, is that the second you stop giving them attention is the second they will stop being your follower.\n\nCritical to note though, is that these users are precisely the ones that make [[Recursive Product Market Fit]] possible. Without having attention seeking users a builder will never know the solution/product they should build. Like a lighthouse in the middle of a storm, [[Attention Seeking Behavior]] from a user is the sign of a problem that should be solved. If you have not found success with an attempt at building a solution it is merely because the incorrect solution has been built.\n\n## Attention Deprevation\n\nUnlocking the power of the [[Network Effects]], utilizing the power of users with [[Attention Seeking Behavior]] is the simplest way to maintain the momentum that has been built in a niche without losing the expertise and understanding that has been established within a single niche.\n\nThankfully, we do know this to be possible and wildly successful outside of Web3, we are just very early still. A great example of this is Pieter Levels who has built within the same niches for years.\n\nInstead, it <author note=\"Revolves around a system where they are the sun of their universe rather than ever having realized how to interact with the galaxies beyond them.\" hover=\"true\">revolves around a system where they are the sun of their universe rather than ever having realized how to interact with the galaxies beyond them.</author>\n\nConstantly evolving his pitch, he is quick to identify problems and deploy possible shapes of the solution. Accepting that nothing is forever, Pieter excels in building products that are made to fit the shape of the active market rather than the shape of his wants. With [[Recursive Product Market Fit]] Pieter has been able to find constant and repeatable success resulting in many believing it to be his following that permits this ability.\n\nHowever that is not the case although a high specialized audience is the outcome of it. Having cultivated an audience of users with high-levels of [[Attention Seeking Behavior]] now countless builders follow and idolize Pieter without ever having realized that his greatest attribute is that he does not give up nor does he believe that his first solution is the only solution.",
        attributes: {
            created: "2023-06-01T00:00:00.000Z",
            readTime: 4,
            authors: [
                "nftchance"
            ],
            tags: [
                "leadership",
                "knowledge"
            ],
            related: [
                "running",
                "black-holes"
            ],
            inbound: [
                "black-holes"
            ]
        }
    },
    {
        filename: "following",
        title: "A Following is not Required",
        description: "Arkhamhttpswwwarkhamintelligencecom has entered the web3 industry without any of its founders having a huge following before or after building Even when the product is rather known today and has gone through multiple rounds of",
        image: "https://i.pinimg.com/600x315/68/3f/0b/683f0bdebe3f585eb5f2b28376427379.jpg",
        content: "[Arkham](https://www.arkhamintelligence.com/) has entered the web3 industry without any of its founders having a huge following before or after building. Even when the product is rather known today and has gone through multiple rounds of funding, the prime founder still only has 500 followers.\n\n![[Pasted image 20221217160013.png]]\n\nThis is just one of many reference points that illustrates multiple points but primarily:\n\n1. Your business can be successful without you being viral, known on Twitter, or really even posting. All that is needed is a good product that can be talked about.\n2. Miguel has 106 tweets and 90% of them are about the product.\n   1. Shitposting can do more harm than good, especially for investors.\n\nMeanwhile, the company Twitter has 37k followers.\n\n![[Pasted image 20221217160231.png]]\n\nThey built a product that users enjoy and value and that has resulted in a community that shows up and supports the company. This is nicely illustrated with the metrics of their **product-focused tweets** such as: https://twitter.com/ArkhamIntel/status/1579812848758587392?s=20&t=YBwR2Sz13K_VUxLdvEfPA\n\n![[Pasted image 20221217160324.png]]\n\nInterestingly, while the company has not strayed away from shitposting, Arkham has also shown that one does not have to be completely serious to market a very serious and very good product.\n\n![[Pasted image 20221217160411.png]]\n\nArkham is a great example of the land that many products aspire to stand in. They have taken a far more meme-heavy route and have a managed person leading the social media now. While that may be the case one day for Cogs, that is not the case on Day 0.\n\nTo siphon the wisdom for Arkham, Cogs will lead the topical and emergent things such as [[Content Topics]] while also utilization [[Content Marketing]] for more long-term and longtail focuses.\n\n> Just because you do not have a following when starting, does not mean your [[Total Addressable Market]] is any smaller than the next guys.",
        attributes: {
            created: "2023-02-01T00:00:00.000Z",
            readTime: 1,
            authors: [
                "nftchance"
            ],
            tags: [
                "thoughts"
            ],
            related: [
                "bigger"
            ],
            inbound: []
        }
    },
    {
        filename: "black-holes",
        title: "Event Horizons or Blackholes",
        description: "Constantly in the land of We Are The Resources one is reminded that success comes from amassing Event Horizons that have an insane amount of gravity When you collect Event Horizons together though you put your business at extreme ris",
        image: "/cdn/black-holes.png",
        content: "Constantly in the land of [[We Are The Resources]] one is reminded that success comes from amassing Event Horizons that have an insane amount of gravity. When you collect Event Horizons together though you put your business at extreme risk.\n\nHow? **Event Horizons create alliances.**\n\nA truly effective Event Horizon has identified that the Network Effect is the most powerful tool they have in their arsenal. Not reliant on any one company or any single idea, an Event Horizon has the true ability to remain nimble (likely because they have always had to be.)\n\nUnfortunately for the company though, Event Horizons assemble to become a Black Hole that destroys everything in its path.\n\n![[Pasted image 20230122105330.png]]\n\nWhile a company views the integration of an Event Horizon as an asset, is an increasing liability that puts the company and all team members at risk with the strong reliance and _key-man_ inclusion that requires significant change when the team disperses.\n\nIn the past week and with the acquired understanding of [[We Are The Resources]] this is already becoming an issue for Flipside and we are only 5 months into employment:\n\nUnfortunately though, this is the perfect display of Event Horizons coming together to create blackholes.\n\nIf something happened to Meg, Drake cannot go back in time and will not take that role which means Drake will begin looking for a new job and figuring out how to adjust to the change without traveling back in time.\n\nFocusing on event horizons to an extreme degree is a severe mistake that only leaders incapable of the process of execution will choose to support.\n\nSupporting event horizons with freedom is not the choice of a smart CEO or one that knows where they are going, they are lost to an extreme degree putting the trust in people they literally cannot comprehend the thoughts of.",
        attributes: {
            created: "2022-10-13T05:00:00.000Z",
            readTime: 1,
            authors: [
                "nftchance",
                "drakedanner"
            ],
            tags: [
                "leadership",
                "knowledge"
            ],
            related: [
                "attention"
            ],
            inbound: [
                "attention"
            ]
        }
    },
    {
        filename: "running",
        title: "Running from Consensus",
        description: "Constantly surrounded by people that do actions merely looking to get a response they must be treated like little siblings A stream of frustration and annoyance those with an Attention Seeking Behavior rarely provide any level of i",
        image: "https://s3.amazonaws.com/s3.timetoast.com/public/uploads/photo/8049975/image/f1339f23a5afa67003fa5109efbd1a21",
        content: "Constantly surrounded by people that do actions merely looking to get a response, they must be treated like little siblings. A stream of frustration and annoyance, those with an [[Attention Seeking Behavior]] rarely provide any level of impact or perspective.\n\nInstead, it revolves around a system where they are the sun of their universe rather than ever having realized how to interact with the galaxies beyond them.\n\nLeft to wonder why they are not like the rest, they resort to [[Attention Seeking Behavior]] because it s the only outlet that continually gets them a response. Yet, the feat they only receive the response only continues to perpetuate the outcome.\n\nAlthough annoying, these people are also a sign that you are reaching the point of escape velocity. For if these people are not around you, you are still miles away from having an audience that is willing to become evangelists.\n\n**How can you easily determine if you are this phase?**\n\n- Are you getting a lot of bug reports that include zero information and just bring a bug like a child?\n- Are you experiencing situations where the user is so incredibly inept that they don't even have the ability to verbalize or communicate an issue?\n- Are you getting people saying things that make absolutely zero sense yet they say it confidently?\n- Are you experiencing large amounts of misperceptions from the average user?\n\nThese are the things that you will not see come in on GitHub. This phase is the one that you will feel deep in your gut that bring the first murmurings of abandoning yet another idea. The type of response and conversations that frustrate you to a feeling that is disgusting and ridiculous.\n\nBut, these displays are precisely why these individuals are participants of your network rather than your network.\n\nInterestingly, frustration at these people shows you still have more growing to do. Vasa of [Gem.xyz](https://gem.xyz) has shown through years of effort and dedication that when you take care of these people, they will take care of you. Important to understand though, is that the second you stop giving them attention is the second they will stop being your follower.\n\nCritical to note though, is that these users are precisely the ones that make [[Recursive Product Market Fit]] possible. Without having attention seeking users a builder will never know the solution/product they should build. Like a lighthouse in the middle of a storm, [[Attention Seeking Behavior]] from a user is the sign of a problem that should be solved. If you have not found success with an attempt at building a solution it is merely because the incorrect solution has been built.\n\nUnlocking the power of the [[Network Effects]], utilizing the power of users with [[Attention Seeking Behavior]] is the simplest way to maintain the momentum that has been built in a niche without losing the expertise and understanding that has been established within a single niche.\n\nThankfully, we do know this to be possible and wildly successful outside of Web3, we are just very early still. A great example of this is Pieter Levels who has built within the same niches for years.\n\nConstantly evolving his pitch, he is quick to identify problems and deploy possible shapes of the solution. Accepting that nothing is forever, Pieter excels in building products that are made to fit the shape of the active market rather than the shape of his wants. With [[Recursive Product Market Fit]] Pieter has been able to find constant and repeatable success resulting in many believing it to be his following that permits this ability.\n\nHowever that is not the case although a high specialized audience is the outcome of it. Having cultivated an audience of users with high-levels of [[Attention Seeking Behavior]] now countless builders follow and idolize Pieter without ever having realized that his greatest attribute is that he does not give up nor does he believe that his first solution is the only solution.",
        attributes: {
            created: "2021-05-09T05:00:00.000Z",
            readTime: 3,
            authors: [
                "drakedanner"
            ],
            tags: [
                "knowledge"
            ],
            inbound: [
                "attention"
            ]
        }
    },
    {
        filename: "bigger",
        title: "Bigger Than You Can Perceive",
        description: "Often times recently many of the fundamental disagreements that I have had with peoples perception of the world comes from them lacking the realization that their are levels of the system higher than they can perceive.",
        image: "https://www.re-thinkingthefuture.com/wp-content/uploads/2020/12/A2477-10-Things-you-did-not-know-about-Roman-Architecture.jpg",
        content: "Often times recently many of the fundamental disagreements that I have had with peoples perception of the world comes from them lacking the realization that their are levels of the system higher than they can perceive.\n\nBelieving the world to be a blue ocean, they are under the impression they can see the whole ocean while only seeing to the horizon.\n\nHolistically, it is simple to identify these people by determining if they believe that things can happen without a certain level of guidance.\n\nIf they say yes, they have a very narrow perspective of the world and they need someone to show them the rails the train runs on.\n\n> One strand of empirical research that is relevant to questions concerning the role of consciousness in agency is the work on automaticity; in particular, the research on automatic goal pursuit. It has been shown, for instance, that the goal to perform a certain task accurately can be primed, so that the agent pursues the goal without any awareness of doing so (Bargh et al. 2001). There is a large body of research on this, and it has been suggested that this research shows that *most* of our actions are executed automatically and without conscious control (Bargh and Chartrand 1999, Custers and Aarts 2010).[[22](https://plato.stanford.edu/entries/agency/notes.html#note-22)] This claim is less radical than the claims put forward by Libet (1999) and Wegner (2002), as it concerns only the extent or scope of conscious control.\n\n> Dual System Theory",
        attributes: {
            created: "2021-01-01T06:00:00.000Z",
            readTime: 1,
            authors: [
                "nftchance"
            ],
            tags: [
                "thoughts"
            ],
            inbound: [
                "following"
            ]
        }
    },
    {
        filename: "capacity",
        title: "Capacity Based Pricing",
        description: "This paper was spawned by the need for usage responsive pricing in the Hyper MetricsDAO modelIn the beginning of the creation of the Labor Market it was originally planned that METRIC would not be used for anything real Instea",
        image: "https://www.worldatlas.com/r/w768/upload/df/d1/ef/shutterstock-88536622.jpg",
        content: "> This paper was spawned by the need for usage responsive pricing in the [[Hyper MetricsDAO]] model.\n\nIn the beginning of the creation of the Labor Market, it was originally planned that $METRIC would not be used for anything real. Instead, with the introduction of `v2` the token became bound to the operational capacity of the protocol.\n\nWhile that can be achieved naively, there is a far more interesting implementation that is using multiple activity curves in combination with a [TWAP model](https://blog.quantinsti.com/twap/).\n\nUnfortunately, Solidity does not have native support for complex numbers and adding support is not exactly straightforward. This brought the research into [[Discerete Cosine Transform]] which aimed to connect a few of the dots and re-discover a new/better way to handle an insane amount of data.\n\nThe aim is to have multiple operational curves for:\n\n- amount of work requests\n- amount of service providers\n- amount of quality enforcers\n\nA critical nuance the ability of using a transform here is that the interval of writing must be constant otherwise everything falls apart rapidly.",
        attributes: {
            created: "2021-01-01T06:00:00.000Z",
            readTime: 1,
            authors: [
                "nftchance"
            ],
            tags: [
                "thoughts"
            ],
            inbound: []
        }
    }
];

const heatmap: HeatmapProps[] = [
    {
        date: "1/2021",
        posts: [
            "bigger",
            "capacity"
        ],
        count: 2
    },
    {
        date: "2/2021",
        posts: [],
        count: 0
    },
    {
        date: "3/2021",
        posts: [],
        count: 0
    },
    {
        date: "4/2021",
        posts: [],
        count: 0
    },
    {
        date: "5/2021",
        posts: [
            "running"
        ],
        count: 1
    },
    {
        date: "6/2021",
        posts: [],
        count: 0
    },
    {
        date: "7/2021",
        posts: [],
        count: 0
    },
    {
        date: "8/2021",
        posts: [],
        count: 0
    },
    {
        date: "9/2021",
        posts: [],
        count: 0
    },
    {
        date: "10/2021",
        posts: [],
        count: 0
    },
    {
        date: "11/2021",
        posts: [],
        count: 0
    },
    {
        date: "12/2021",
        posts: [],
        count: 0
    },
    {
        date: "1/2022",
        posts: [],
        count: 0
    },
    {
        date: "2/2022",
        posts: [],
        count: 0
    },
    {
        date: "3/2022",
        posts: [],
        count: 0
    },
    {
        date: "4/2022",
        posts: [],
        count: 0
    },
    {
        date: "5/2022",
        posts: [],
        count: 0
    },
    {
        date: "6/2022",
        posts: [],
        count: 0
    },
    {
        date: "7/2022",
        posts: [],
        count: 0
    },
    {
        date: "8/2022",
        posts: [],
        count: 0
    },
    {
        date: "9/2022",
        posts: [],
        count: 0
    },
    {
        date: "10/2022",
        posts: [
            "black-holes"
        ],
        count: 1
    },
    {
        date: "11/2022",
        posts: [],
        count: 0
    },
    {
        date: "12/2022",
        posts: [],
        count: 0
    },
    {
        date: "1/2023",
        posts: [
            "following"
        ],
        count: 1
    },
    {
        date: "2/2023",
        posts: [],
        count: 0
    },
    {
        date: "3/2023",
        posts: [],
        count: 0
    },
    {
        date: "4/2023",
        posts: [],
        count: 0
    },
    {
        date: "5/2023",
        posts: [
            "attention"
        ],
        count: 1
    },
    {
        date: "6/2023",
        posts: [],
        count: 0
    }
];

export const usePosts = (): PostProps[] => {
    return directory;
}

export const usePost = (filename: string): PostProps | undefined => {
    return directory.find((post) => post?.filename === filename);
}

export const useHeatmap = (): HeatmapProps[] => {
    return heatmap;
}