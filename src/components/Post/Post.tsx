...
import { Container, HalftoneCard, Header, Meta, PostCard, PostHeader, PostInfo, PostMarkdown, Tag } from "../"
		...
		<>
			...
			<Container className="thin post">
				...
				<div className="image">
					...
					{data.attributes.related && data.attributes.related.length > 0 && (
						<Link to={`/post/${data.attributes.related?.[0]}/?from=${id}`} className="prev">
							<FontAwesomeIcon icon={faChevronLeft} />
						</Link>
					)}

					{data.attributes.inbound && data.attributes.inbound.length > 0 && (
						<Link to={`/post/${data.attributes.inbound?.[0]}/?from=${id}`} className="next">
